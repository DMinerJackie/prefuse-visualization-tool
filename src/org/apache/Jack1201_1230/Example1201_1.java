 ...
 * 增加高亮显示NeighborHighlightControl
 * 利用中间键控制平移
 * 利用左键控制视图旋转
 * 添加SubtreeDragControl控制器
 * @author Jack
 *
 */
public class Example1201_1{
...
//    	edgeI.setSize(8);
		...
		display.addControlListener(new PanControl(Control.MIDDLE_MOUSE_BUTTON));//利用鼠标中间键控制平移功能
		display.addControlListener(new ZoomControl());
		display.addControlListener(new WheelZoomControl());
		display.addControlListener(new ZoomToFitControl());
		display.addControlListener(new FocusControl(1, "shape"));//在击中某个ViusalItem后实现变形的功能
		display.addControlListener(new NeighborHighlightControl());
		display.addControlListener(new RotationControl(Control.LEFT_MOUSE_BUTTON));
