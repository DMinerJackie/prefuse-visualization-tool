 ...
 */
public class Example1212 extends Display {
    
    public Example1212(Table t) {
        super(new Visualization());
        m_vis.addTable("data", t);//添加表格
        
        LabelRenderer nodeRenderer = new LabelRenderer(null, "image");
        nodeRenderer.setTextField(null);//设置文本域为空
        nodeRenderer.setVerticalAlignment(Constants.BOTTOM);
        nodeRenderer.setHorizontalPadding(0);
        nodeRenderer.setVerticalPadding(0);
        nodeRenderer.setMaxImageDimensions(100,100);//设置显示图像的大小
        
        m_vis.setRendererFactory(new DefaultRendererFactory(nodeRenderer));
        
        ActionList init = new ActionList();
        init.add(new CircleLayout("data"));//这里改为圆形布局
        init.add(new DataMountainSizeAction());//设置每幅图的大小
        ...
        update.add(new ColorAction("data", VisualItem.STROKECOLOR) {
            public int getColor(VisualItem item) {
                return ColorLib.rgb((item.isHover() ? 255 : 0), 0, 0);//此等写法实在妙不可言
            }
        });
    ...
    public static JComponent demo() {
        String datafile = "E:\\workspace0428\\MySwingTest\\src\\org\\apache\\widget\\amazon.txt";
        Table data = null;
        try {
            data = (new DelimitedTextTableReader()).readTable(datafile);
            data.addColumn("image","CONCAT('src/org/apache/widget/images/',id,'.01.MZZZZZZZ.jpg')");//新添加一列
        } catch (Exception e) {
            e.printStackTrace();
            System.exit(1);
        }
        return new Example1212(data);
    }
    ...
    public class DataMountainSizeAction extends SizeAction {
        public double getSize(VisualItem item) {
            ...
            System.out.println(item.toString()+"\nstarty:"+item.getStartY()+"endY:"+y+"startX:"+item.getStartX()+"endX:"+item.getEndX());//打印出每个图片的位置信息
            return 0.2 + y/1400;//控制图片的大小以及逐渐变大的效果  去掉"0.2+" 效果更明显
        }
    ...
    public class DataMountainForceLayout extends ForceDirectedLayout {//定制自己的布局方法
        ...
        public DataMountainForceLayout(boolean enforceBounds) {
            ...
            fsim.addForce(new SpringForce(1e-5f,0f));//喷泉模拟器
    ...
    public class DataMountainControl extends ControlAdapter {
        ...
        public void itemEntered(VisualItem item, MouseEvent e) {//鼠标经过时显示标签文本
            Display d = (Display)e.getSource();
            d.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));//当鼠标悬浮时会出现手型形状
