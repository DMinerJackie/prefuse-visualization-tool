...
public class LabelRenderer extends AbstractShapeRenderer {
	...
	protected Shape getRawShape(VisualItem item) {
		...
		item.setStrokeColor(ColorLib.rgb(110,0,0));//为图形添加描边
		
		Node node = (Node)item;
		int degreeNum = node.getDegree();//近邻节点个数
		if(degreeNum <= 0){//如果degreeNum为0，则在下面的log函数中会报错
			degreeNum = 1;
		}
		...
		case Constants.RIGHT:
			w = Math.log(degreeNum)*10 + size * (iw + 2 * m_horizBorder + (tw > 0 && iw > 0 ? m_imageMargin : 0));
//			w = tw + size * (iw + 2 * m_horizBorder + (tw > 0 && iw > 0 ? m_imageMargin : 0));
			h = Math.log(degreeNum)*10 + size * 2 * m_vertBorder;
			break;
		case Constants.TOP:
