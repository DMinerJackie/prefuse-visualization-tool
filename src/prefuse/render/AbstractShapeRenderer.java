...
//		System.out.println("weight: \t" + item.get(GraphMLHandler.WEIGHT));
		
		if (shape != null)
		{
//			item.setStrokeColor(ColorLib.rgb(0,100,0));//为图形添加描边
...
//			}
//			if(Integer.parseInt(item.get(GraphMLHandler.WEIGHT).toString())>0){
//				
//			}
			try {
				weight = item.get(GraphMLHandler.WEIGHT).toString();
