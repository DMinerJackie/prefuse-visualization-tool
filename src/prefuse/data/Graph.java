...
public class Graph extends CompositeTupleSet {
	...
	public static final String DEFAULT_DIRECTED_KEY = PrefuseConfig.get("data.graph.directedKey");
	...
	public Edge addEdge(Node s, Node t) {
		nodeCheck(s, true);
		nodeCheck(t, true);
		// System.out.println(s.get("name") + "->" + t.get("name"));
	...
	/**
	 * 设置边的默认方向性，true-全图方向，false-全图无方向，NULL-默认
	 * 
	 * @param directed
	 */
	...
	public Boolean isDirected() {
		return m_directed;//全局的m_directed有两种来源:1.读取xml文件中的edgedefault进行赋值,这在GraphMLReader中有体现; 2.通过下拉框选择面板进行传值,这在GraphUtils中有体现
	}
	...
	public Iterator tuples(Predicate filter) {
		if (filter == null) {
			...
			return new CompositeIterator(m_edgeTuples.iterator(getEdgeTable().rows(filter)), m_nodeTuples.iterator(getNodeTable().rows(filter)));
		}
	}
	 ...
	 * spanning tree is already rooted at the given node, it is simply returned.
